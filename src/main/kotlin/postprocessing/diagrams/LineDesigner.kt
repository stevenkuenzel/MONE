...
/**
 * Creates line designs based on the features of their respective sources.
 */
class LineDesigner {
    companion object {
         ...
         */
        private val INDEX_COLORS = arrayOf(
        ...
        val colors = createColors(512)
        val shapes = createShapes()
        val strokes = createStrokes()

        fun getLineDesign(data: Array<Int>): LineDesign {
            assert(data.size == 3)
            ...
            return LineDesign(colors[data[0]], shapes[data[1]], strokes[data[2]])
        }

        private fun createColors(amount_: Int): Array<ColorSet> {
            val amount = if (amount_ <= INDEX_COLORS.size) amount_ else INDEX_COLORS.size

            return Array(amount) { i -> ColorSet(Color.decode(INDEX_COLORS[i])) }
        }

        private fun createShapes(): Array<Shape> {
            val shapes = ArrayList<Shape>()
            val w = 4.5f

            shapes.add(Ellipse2D.Float(-w, -w, 2 * w, 2 * w))
            shapes.add(RoundRectangle2D.Float(-w, -w, 2 * w, 2 * w, 2f, 2f))
            shapes.add(ShapeUtils.rotateShape(shapes[shapes.size - 1], Math.PI * 0.25, 0f, 0f))
            shapes.add(ShapeUtils.createRegularCross(w, w))
            shapes.add(ShapeUtils.createDiamond(w))
            shapes.add(ShapeUtils.createUpTriangle(w))
            shapes.add(ShapeUtils.rotateShape(shapes[shapes.size - 1], Math.PI * 0.5, 0f, 0f))
            shapes.add(ShapeUtils.rotateShape(shapes[shapes.size - 1], Math.PI * 0.5, 0f, 0f))
            shapes.add(ShapeUtils.rotateShape(shapes[shapes.size - 1], Math.PI * 0.5, 0f, 0f))
            shapes.add(Ellipse2D.Float(-w, -w / 2, 2 * w, w))
            shapes.add(Ellipse2D.Float(-w / 2, -w, w, 2 * w))

            return shapes.toTypedArray()
        }


        private fun createStrokes(): Array<FloatArray> {
            val strokes = ArrayList<FloatArray>()

            strokes.add(floatArrayOf(2000f))
            val len = 5f

            for (i in 1..4) {
                strokes.add(floatArrayOf(i * len, i * len))
                strokes.add(floatArrayOf(i * len * 3, len, 3f, len))
            }
